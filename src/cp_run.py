...
def main():
    ...
    mlflow.set_experiment(experiment_name="Basic CP")
    ...
    with mlflow.start_run(run_name=name):
        ...
        study = optuna.create_study(direction="maximize", study_name="Basic CP")
        study.optimize(lambda trial: objective(trial, T, L, events), n_trials=n_trials)
        mlflow.log_params(study.best_params)
    mlflow.log_params(study.best_params)
    mlflow.log_metric("best_obj", study.best_value)
