...
def compute_tensor_model_metrics(
    ...
    # ----------------------------
    # Threshold selection (if not given)
    # ----------------------------
...
def compute_binary_model_metrics(
    y_pred: np.ndarray,
    y_true: np.ndarray,
) -> Metrics:
    """
    Compute evaluation metrics for binary predictions (0/1).
    ...
    return Metrics(
        n_anomalies=int(np.sum(y_true)),
        roc_auc=None,  # Not defined without probabilities
        pr_auc=None,  # Not defined without probabilities
        precision=precision_val,
        recall=recall_val,
        f1=f1_val,
        fpr=fpr_val,
        tpr=tpr_val,
        threshold=None,  # No threshold used
        TP=int(tp),
        FP=int(fp),
        TN=int(tn),
        FN=int(fn),
